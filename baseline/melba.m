...
%	SLIST	- cell array of msec offsets {length(vList)}[1 x nLabs], created as anon. labels
...
% load formant data if available ("FMTS*")
	k = strmatch('FMTS',{data.NAME});
	if ~isempty(k),
		if nargin > 3 && ischar(varargin{2}) && strcmp(varargin{2},'CONFIG'),
			cfg = varargin{3};
			lpState = cfg.LPSTATE;
			if ~isempty(lpState) && strcmp(lpState.SOURCE,'LP_RMS'),
				k = strmatch(lpState.FMTS,{data.NAME},'exact');
			end;
		end;
		formants = data(k(1)).SIGNAL;
		daqData = 1;
	else,
		formants = [];
	end;

% load F0 data if available ("F0*")
	k = strmatch('F0',{data.NAME});
	if ~isempty(k),
		if nargin > 3 && ischar(varargin{2}) && strcmp(varargin{2},'CONFIG'),
			cfg = varargin{3};
			lpState = cfg.LPSTATE;
			if ~isempty(lpState) && strcmp(lpState.SOURCE,'LP_RMS'),
				k = strmatch(lpState.F0,{data.NAME},'exact');
			end;
		end;
		f0 = data(k(1)).SIGNAL;
		daqData = 1;
...
%				set(state.CURSORL, 'xData', [x x]);		% update cursor position
				...
				if xy>0 & xy <state.SRATE/2,	% click within spectrogram
					...
					xorline([xy xy], get(state.SPECTRA, 'ylim'), 'parent', state.SPECTRA, 'tag', 'CURSOR', 'color', 'c');
				end;
				set(gcbf, 'userdata', state, ...
...
% click in spectrum panel reports location
			case 'SPECTRA',
				...
				set(state.HEADF, 'string', sprintf(' %d', round(x)));
...
% compute formants
		...
		fprintf('\n\tCursor @ %g ms\nWindow %.1f ms:  %d zero crossings, RMS = %.1f (%.1f dB), F0 = %d Hz\nFormants (Bandwidths, Amplitudes):\n  [', ...
				state.CURSOR, state.WSIZE, zc, rms, 20*log10(rms), F0);
...
% show canned values, if any
		if ~isempty(state.F0),
			k = c/length(state.SIGNAL) * length(state.F0);
			k1 = floor(k);
			k2 = k1 + 1;
			if k1 < 1, k1 = 1; elseif k2 > length(state.F0), k2 = length(state.F0); end;
			v = state.F0(k1:k2);
			v = v(1) + diff(v)*(k-k1);
		...
		if ~isempty(state.FORMANTS),
			...
			k = c/length(state.SIGNAL) * lf;
			k1 = floor(k);
			k2 = k1 + 1;
			if k1 < 1, k1 = 1; elseif k2 > lf, k2 = lf; end;
			v = state.FORMANTS(k1:k2,1:nf);
			v = v(1,:) + diff(v) * (k-k1);
...
% reload labels from <NAME>_lbl if defined in base workspace
		...
		k = findstr(ns,'.');
		ns(k) = '_';
		if isempty(labels) && evalin('base',sprintf('exist(''%s_lbl'',''var'')',ns)),
			labels = evalin('base',[ns '_lbl']);
			labels = labels(:)';
		end;
...
% framing panel
		...
		hold on;
		ec = [.25 .25 .25];
		sh = patch([1 1 nSamps nSamps], [-1 1 1 -1]*spread, ec, 'EdgeColor', ec);
		if verLessThan('matlab','8.4.0'), set(sh, 'eraseMode','xor'); else, set(sh, 'faceAlpha',.6); end
		hold off;
...
% RMS panel
		...
		rms = sqrt(abs(filtfilt(double(2*hamming(wl)/wl),1,signal(:,1).^2)));
		rms = (rms - min(rms));
		rms = rms / max(rms);
		plot(rms, 'color', [1 1 .8]);
		set(rmsa, 'xticklabel', [], 'ytick', [], 'xlim', [1 nSamps], 'ylim', [0 1.1]);
		ylabel('RMS');
...
%		ra = axes('position', [.575 .01 .42 .15], ...	% spectra axis
		...
		rl = xorline(0, 0, 'color', 'w', 'buttonDownFcn', 'melba(''DOWN'',''SPECTRA'')');
...
%		za = axes('position', [.42 .01 .149 .15], ...
		...
		zoom = xorline(0,0,'color','w', 'userData', [-maxV maxV], 'buttonDownFcn', 'melba(''ZOOM'',''PLOT'')');
...
% MELBA menu			
		...
		if isempty(cfg.AUTO),
			if nSamps > 1e6, auto=0; cs='off'; else, auto=1; cs='on'; end;
		else,
			auto = cfg.AUTO;
...
% Variable List menu
		if ~isempty(vList),
			...
			uimenu(menu, 'label', 'Next plus export', ...
				'accelerator', '4', ...
				'callback', 'melba(''LEXPORT'');melba(''VARLIST'',1);');
			...
			uimenu(menu, 'label', 'Next; export/save labs, close current', ...
				'accelerator', '7', ...
				'callback', 'melba(''LEXPORT'');melba(''LSAVE'',1);melba(''VARLIST'',1);close(gcbf);');
...
% these assignments must be done separately to avoid cloning state
		if isempty(vList),
			if ~isempty(cfg.SLIST) && ~iscell(cfg.SLIST), cfg.SLIST = {cfg.SLIST}; end;
		else,
			state.VLIST = vList;
			if length(cfg.SLIST)~=length(vList), cfg.SLIST = {}; end;
		end;
		if ~isempty(cfg.SLIST), state.SLIST = cfg.SLIST; end;
		state.LABELS = labels;
		...
		if ~isempty(labels),
			...
			for i = 1 : length(labels),
				if isempty(cfg.LPROC) || isempty(cfg.LPSTATE),
					x = labels(i).OFFSET;
					labels(i).HANDS = xorline([x x], y, 'tag', 'LABEL', 'buttonDownFcn', 'melba(''LMOVE'',''DOWN'');');
...
% 'superimposed' labels (SLIST)
		if ~isempty(state.SLIST),
			axes(state.CURSORH);
			y = get(state.CURSORH, 'yLim') * .98;
			if isempty(vListSel), vListSel = 1; end;
			sl = state.SLIST{vListSel};
			x = [1;1] * sl(:)';
...
% config label proc if necessary
		...
		if ~isempty(cfg.LPROC) && isempty(cfg.LPSTATE),
			mOut = feval(cfg.LPROC, [], 'CONFIG', state);	% call user proc config handler
			if iscell(mOut), lpState = mOut{1}; else, lpState = mOut; end;
			if ~isempty(lpState),
				if isnumeric(lpState) && lpState == -1,		% call label procedure immediately
					...
					if length(mOut) > 1, state = mOut{2}; end;
...
%
%	melba('LCEAR',1) clears all labels w/o dialog

	case 'LCLEAR',
		if nargin>1 || strcmp(questdlg('Clear all labels...', 'Verify...', 'Yes', 'No', 'Yes'), 'Yes'),
			state = get(gcbf, 'userdata');
			state.LABELS = [];
			set(gcbf, 'userdata', state);
	...
	case 'LPLOT',
		...
		if ~isempty(label.NAME),
			label.HANDS(end+1) = text(x(2), ylim(2), [' ', label.NAME], ...
				...
				'fontname', 'geneva', ...
				'fontsize', 9);
			if verLessThan('matlab','8.4.0'), set(label.HANDS(end),'eraseMode','xor'); end
		end;
		mOut = label;
	...
	case 'LSETPROC',
		...
		switch varargin{2},
			...
			case 1,			% set user proc
				[file, p] = uigetfile([fileparts(which('melba')),filesep,'lp*.m'], 'Select labelling procedure');
				if file == 0, return; end; 	% cancelled
				i = findstr(file, '.');
				if ~isempty(i), file = file(1:i(1)-1); end;
				mOut = feval(file, [], 'CONFIG',state);	% call user proc config handler (1st time)
				if iscell(mOut), newState = mOut{1}; else, newState = mOut; end;
			...
			case 2,			% reconfigure
				...
				mOut = feval(file, state.LPSTATE, 'CONFIG',state);	% call user proc config handler
				if iscell(mOut), newState = mOut{1}; else, newState = mOut; end;
				if isempty(newState), return; end;	% cancelled
				if length(mOut) > 1, state = mOut{2}; end;
				state.LPSTATE = newState;
		...
		set(gcbf, 'userdata', state);
		if refresh, SetBounds(state,0); end;
	...
	case 'MOVECUR',
		...
		if ~isempty(lh),
			...
			if all([x>=p(1) , x<=p(2) , y>=p(3) , y<=p(4)]),
				set(state.HEADF, 'string', sprintf(' %d',round(y)));
				set(lh, 'xdata', [y y]);
			else,
...
% want non-blocking audio output
		sel = state.SIGNAL(hs:ts) / abs(state.SPREAD);
		if isunix,
			try,
				evalin('base','stop(PLAYER)');		% abort ongoing output
			catch,
				assignin('base','PLAYER', audioplayer(sel, state.SRATE));
				evalin('base','set(PLAYER,''stopFcn'',''melba PLAYX'');play(PLAYER)');
				return;
			end
			evalin('base','clear PLAYER');
		else,
			soundsc(sel, state.SRATE, [-1 1]*abs(state.SPREAD));
		end;
...
% adjust label offsets to start of selection
		...
		if isempty(state.DATA),
			sel = struct('NAME','AUDIO', ...
							...
							'SRATE', state.SRATE, ...
							'LABELS', labels, ...
							'SOURCE', struct('NAME',state.NAME,'HEAD',head,'TAIL',tail));
% 			if ~isempty(state.F0),
...
% SAVESELX:  export current selection as MS WAV file
	...
	case 'SAVESELX',
		state = get(gcbf, 'userdata');
		[p,f] = fileparts(state.NAME);
		name = GetName([f '_sel.wav'], 'selection', 'Export @ to MS WAV file');
		if isempty(name) return; end;		% cancel
		...
		if range(s) < 2,
			try,
				audiowrite(s, state.SRATE, name);		% +/- 1 (WAVE-scaled)
			catch,
				wavwrite(s, state.SRATE, name);
			end;
		elseif range(s) > 4096,
			WriteSound(s, state.SRATE, name, 'wav', 32768);	% 16 bit data
		else,
...
%		vis = umtoggle(varargin{2});	% toggle menu
		...
		if isempty(state.SPATWIN) | ~ishandle(state.SPATWIN),		% initialize
			...
			if ~isempty(state.SPLINE),
				h(nTraj+1) = xorline(x, y, 'hitTest','off','color',[.5 .5 .5]);
			end;
...
offs = sort(cell2mat({labels.OFFSET}));
if length(offs) < 2, return; end;
n = 1;
while n < length(offs),
	if cursor>=offs(n) & cursor<=offs(n+1), break; end;
	n = n + 1;
end;
if n == length(offs), return; end;	% no bracketing pair
h = offs(n);
t = offs(n+1);
...
if ~isempty(state.F0), 
	...
	idx = isnan(f0);
	f0(idx) = 0;
	k = linspace(1,length(f0),size(state.SIGNAL,1));
	f0 = interp1(f0,k);
	idx = interp1(double(idx),k);
	f0(find(idx>0)) = NaN;
	plot(state.F0A, f0, 'c');
	set(state.F0A, 'xticklabel',[],'xlim',[hs ts],'ylim',[nanmin(f0)-10 nanmax(f0)+20]);
end;	
axes(state.CURSORH);
